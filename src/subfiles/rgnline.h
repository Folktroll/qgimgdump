...
class RgnLine {
 public:
  ...
  bool hasNet1Label = false;  // true if label offset has to be used in NET subfile (up to four label pointers), @todo: missing labels
  QPolygonF points;           // the actual polyline points as longitude / latitude [rad]
  QStringList labels;
  quint8 hasExtLabelCount = 0;
  bool hasExtLabel = false;  // ext label
